...
describe AttrAccessibleBlock do
  ...
  it "should support #attr_accessible? with multiple attr_accessible" do
    user = User.new(:name => "User Test", :email => 'test@test.com', :password => 'test', 'date(i0)' => '10', :role => 'admin')
    expect(user.attr_accessible?(:name)).to eq true
    expect(user.attr_accessible?(:email)).to eq true
    expect(user.attr_accessible?(:password)).to eq true
    expect(user.attr_accessible?(:date)).to eq true
    expect(user.attr_accessible?(:role)).to eq false
  end
  ...
  it "should have block attr_accessible" do
    location = Location.new(:name => 'test', :size => 10, :user => 'user', 'point(i2)' => '10')
    expect(location.attr_accessible?(:name )).to eq true
    expect(location.attr_accessible?(:size )).to eq true
    expect(location.attr_accessible?(:user )).to eq false
    expect(location.attr_accessible?(:point)).to eq false
  end
